...
<template>
  <div>
    ...
    <div v-if="auth.isManager" class="mb-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
      <StatCard title="Tổng nhân viên" :value="totalEmployees" subtitle="Trong hệ thống" color="emerald" :loading="loading">
        <template #icon>
          <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
        </template>
      </StatCard>
      <StatCard title="Đang làm việc" :value="activeEmployees" subtitle="Trạng thái Active" color="blue" :loading="loading">
        <template #icon>
          <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
        </template>
      </StatCard>
      <StatCard title="Phòng ban" :value="totalDepts" subtitle="Đang hoạt động" color="violet" :loading="loading">
        <template #icon>
      ...
      <StatCard title="Chờ duyệt nghỉ" :value="pendingLeaves.length" subtitle="Đơn nghỉ phép" color="amber" :loading="loading">
        <template #icon>
    ...
    <div v-if="auth.isManager" class="grid grid-cols-1 gap-6 lg:grid-cols-2">
      <!-- Department breakdown -->
      <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        <div class="mb-4 flex items-center justify-between">
          <h2 class="text-sm font-semibold text-slate-800">Nhân viên theo phòng ban</h2>
        ...
        <div v-else class="space-y-2">
          ...
          >
            <span class="text-sm text-slate-600">{{ dept.name }}</span>
            <div class="flex items-center gap-3">
              <div class="w-24 h-1.5 rounded-full bg-slate-100 overflow-hidden">
                <div class="h-full rounded-full bg-emerald-500" :style="{ width: `${(dept.count / totalEmployees) * 100}%` }" />
              </div>
              <span class="text-sm font-semibold text-slate-800 w-6 text-right">{{ dept.count }}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        <div class="mb-4 flex items-center justify-between">
          <h2 class="text-sm font-semibold text-slate-800">Đơn nghỉ phép chờ duyệt</h2>
        ...
        <div v-else class="space-y-2">
          <div
            ...
            :key="lv.id"
            class="flex items-center justify-between rounded-lg bg-amber-50 border border-amber-100 px-3 py-2"
          >
            <div>
              <div class="text-sm font-medium text-slate-800">{{ lv.employeeName }}</div>
