...
<template>
  <div>
    ...
    <!-- Navigation Bar for Weekly Grid View -->
    <div v-if="viewMode === 'grid'" class="flex flex-wrap items-center justify-between gap-4 mb-5 bg-white p-4 rounded-2xl border border-slate-150 shadow-sm">
      <div class="flex items-center gap-2">
        <button @click="prevWeek" class="h-9 w-9 flex items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-600 hover:text-emerald-600 hover:border-emerald-300 hover:bg-emerald-50/20 transition-all">
          <ChevronLeft class="h-5 w-5" />
        </button>
        <button @click="goToday" class="h-9 px-4 rounded-xl border border-slate-200 bg-white text-sm font-semibold text-slate-700 hover:text-emerald-600 hover:border-emerald-300 hover:bg-emerald-50/20 transition-all">
          Tuần này
        </button>
        <button @click="nextWeek" class="h-9 w-9 flex items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-600 hover:text-emerald-600 hover:border-emerald-300 hover:bg-emerald-50/20 transition-all">
          <ChevronRight class="h-5 w-5" />
        </button>
      </div>
      <div class="text-sm font-bold text-slate-800 flex items-center gap-2.5 bg-slate-50 px-4 py-2 rounded-xl border border-slate-100">
        <Calendar class="h-4.5 w-4.5 text-emerald-600" />
        <span>Tuần từ <span class="text-emerald-700 font-extrabold">{{ fmt(formatDateToYMD(currentWeekDays[0])) }}</span> đến <span class="text-emerald-700 font-extrabold">{{ fmt(formatDateToYMD(currentWeekDays[6])) }}</span></span>
      </div>
    </div>
    ...
    <div class="flex flex-wrap items-center gap-4 mb-6 bg-slate-50 p-4 rounded-2xl border border-slate-150 shadow-sm">
      <div class="flex-1 min-w-[280px]">
        <label class="text-[11px] font-bold text-slate-400 uppercase tracking-wider block mb-1.5">Tìm kiếm</label>
        <div class="relative">
          <input
            v-model="search"
            type="text"
            placeholder="Tìm kiếm theo nhân viên, ca làm hoặc ngày..."
            class="h-10 w-full rounded-xl border border-slate-250 bg-white px-3.5 pl-10 text-sm outline-none transition-all focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 placeholder:text-slate-400"
          />
      ...
      <!-- Lọc phòng ban (chỉ hiển thị cho vai trò quản lý/HR/admin hoặc PayrollStaff) -->
      <div v-if="auth.isManager || auth.isPayrollStaff" class="w-full sm:w-auto sm:min-w-[220px]">
        <label class="text-[11px] font-bold text-slate-400 uppercase tracking-wider block mb-1.5">Phòng ban</label>
        <div class="relative">
          <select
            v-model="selectedDeptId"
            class="h-10 w-full rounded-xl border border-slate-250 bg-white px-3.5 pr-10 text-sm outline-none transition-all focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 appearance-none text-slate-700"
          >
      ...
      <!-- Lọc trạng thái (chỉ áp dụng ở dạng danh sách List View) -->
      <div v-if="viewMode === 'list'" class="w-full sm:w-auto sm:min-w-[180px]">
        <label class="text-[11px] font-bold text-slate-400 uppercase tracking-wider block mb-1.5">Trạng thái</label>
        <div class="relative">
          <select
      ...
      <!-- Sắp xếp theo thời gian (chỉ áp dụng ở dạng danh sách List View) -->
      <div v-if="viewMode === 'list'" class="w-full sm:w-auto sm:min-w-[180px]">
        <label class="text-[11px] font-bold text-slate-400 uppercase tracking-wider block mb-1.5">Sắp xếp theo ngày</label>
        <div class="relative">
          <select
            v-model="sortByDate"
            class="h-10 w-full rounded-xl border border-slate-250 bg-white px-3.5 pr-10 text-sm outline-none transition-all focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 appearance-none text-slate-700"
